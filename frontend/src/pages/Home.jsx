...
export default function Home({ goTo }) {
  ...
  const { role, user } = useAuth(); // useAuth'dan gelen bilgiler
    ...
    <div className="flex flex-col gap-8 pb-10 transition-colors duration-300 dark:text-slate-200">
      ...
      <section className="text-center max-w-3xl mx-auto mt-6">
        ...
        <div className="flex justify-center gap-4">
          <button onClick={() => goTo('operations')} className="bg-leaf text-white px-6 py-3 rounded-xl font-semibold hover:bg-leaf/90 transition-all shadow-md">
            Operasyon
          </button>
          <button onClick={() => goTo('riskMap')} className="bg-white dark:bg-slate-800 text-ink dark:text-white border border-[#dfe8df] dark:border-slate-700 px-6 py-3 rounded-xl font-semibold hover:bg-gray-50 dark:hover:bg-slate-700 transition-all">
            Risk Haritası
          </button>
        </div>
      </section>

      {/* 👤 Katman 1.5: HESABIM (Sadece Admin veya Cooperative ise görünür) */}
      {role && role !== 'public' && (
        <section className="max-w-7xl mx-auto w-full px-4 md:px-0">
          <div className="bg-white dark:bg-slate-900 border border-mist dark:border-slate-800 rounded-2xl p-6 shadow-sm flex flex-col md:flex-row items-center justify-between gap-4 transition-colors duration-300">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h2 className="text-xl font-bold text-ink dark:text-white">
                  Hoş Geldin, {role === 'admin' ? 'Yönetici' : user?.name || 'Yeşil Kooperatif'}
                </h2>
                <p className="text-sm text-moss dark:text-slate-400 font-medium uppercase tracking-wider">
                  Yetki: <span className="text-leaf dark:text-emerald-400">{role}</span>
                </p>
              </div>
            </div>
            <div className="flex gap-6 text-center">
              <div>
                <p className="text-xs text-moss dark:text-slate-400 uppercase tracking-wider font-semibold mb-1">Yeşil Puanım</p>
      ...
      <section className="bg-gradient-to-br from-emerald-50 to-[#fffaf0] dark:from-slate-900 dark:to-slate-800 border border-mist dark:border-slate-800 rounded-2xl p-6 transition-colors duration-300 mx-4 md:mx-0">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="bg-white dark:bg-slate-800 p-5 rounded-xl border border-gray-100 dark:border-slate-700 shadow-sm">
            ...
            <div className="space-y-3">
              {podium.map((coop, index) => (
                <div key={index} className="flex justify-between items-center p-3 bg-gray-50 dark:bg-slate-900/50 rounded-lg">
          ...
          <div className="space-y-4">
            {trustInsights.map((insight, idx) => (
              <div key={idx} className="bg-white dark:bg-slate-800 p-4 rounded-xl border border-gray-100 dark:border-slate-700 shadow-sm flex flex-col gap-2">
      ...
      {/* ⚙️ Katman 4: Operasyonel Derinlik */}
      <section className="grid grid-cols-1 lg:grid-cols-3 gap-6 px-4 md:px-0">
        <div className="lg:col-span-2 bg-white dark:bg-slate-900 border border-mist dark:border-slate-800 rounded-2xl p-6 shadow-sm">
             ...
             <table className="w-full text-left text-sm">
               <thead>
                 <tr className="text-moss dark:text-slate-400 border-b border-gray-100 dark:border-slate-700">
                   ...
                   <th className="pb-3 font-semibold text-right">Yeşil Puan</th>
                 </tr>
               </thead>
               <tbody>
                 {rows.slice(0, 5).map((row, i) => (
                   <tr key={i} className="border-b border-gray-50 dark:border-slate-800/50 last:border-0 hover:bg-gray-50/50 dark:hover:bg-slate-800/30 transition-colors">
                     ...
                     <td className="py-3 text-leaf dark:text-emerald-400 font-bold text-right">{row.green_score}</td>
                   </tr>
                 ))}
               </tbody>
             </table>
        ...
        <div className="bg-white dark:bg-slate-900 border border-mist dark:border-slate-800 rounded-2xl p-6 shadow-sm">
